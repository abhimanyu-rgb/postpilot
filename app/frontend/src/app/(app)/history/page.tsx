...
// History only shows terminal outcomes — in-flight states (pending_review,
// approved, queued) live on the Review Queue tab.
type ViewMode = "all" | "published" | "rejected";
...
export default function HistoryPage() {
  ...
  // History shows terminal outcomes only — published, rejected, archived.
  // In-flight states (pending_review, approved, queued) live in Review Queue.
  ...
  async function refetchDrafts() {
    try {
      ...
      setDrafts(fresh.filter((d) => TERMINAL_STATUSES.has(d.status)));
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-lg p-1 w-fit">
        {(["all", "published", "rejected"] as ViewMode[]).map((f) => (
                        ...
                        <div key={draft.id} className={isArchived ? "opacity-40" : ""}>
                          ...
                          <div className={`px-4 py-2.5 flex items-center gap-3 ${isSelected ? "bg-indigo-50/40" : "hover:bg-violet-50/20"}`}>
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              ...
                              {/* Revert to review (rejected only — terminal but reversible) */}
